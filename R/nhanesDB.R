...
.nhanesTablesDB <-
           ...
           namesonly = FALSE, includerdc = FALSE )
  {
    ...
    query = metadata_questionnaire_descriptions |>
      dplyr::filter(DataGroup == data_group, BeginYear == if (EVEN) year - 1 else year) |>
      dplyr::mutate(Component = concat(substr(DataGroup, 1, 1), tolower(substr(DataGroup, 2, 20))))
                               ...
                               nchar=128, namesonly = FALSE)
{
  ...
  metadata_questionnaire_descriptions = dplyr::tbl(cn(), I(MetadataTable("QuestionnaireDescriptions")))
  metadata_questionnaire_variables = dplyr::tbl(cn(), I(MetadataTable("QuestionnaireVariables")))
  ...
  query = metadata_questionnaire_variables |>
    ...
    dplyr::mutate(Component = paste0(substr(DataGroup, 1, 1), tolower(substr(DataGroup, 2, 20)))) |>
  ...
  if (!is.null(data_group)) {
    query = query |> dplyr::filter(str_detect(tolower(DataGroup),tolower(data_group)))
  }
...
{
  .checkTableNames(nh_table)
  # Define the table reference using MetadataTable
  metadata_questionnaire_variables = dplyr::tbl(cn(), I(MetadataTable("QuestionnaireVariables")))
  
  # Build the label query using dplyr
  label_query = metadata_questionnaire_variables |>
    dplyr::filter(TableName == nh_table) |>
    dplyr::select(Variable, Description)
  
  # Execute the label query and fetch the results
  var_label = label_query |> dplyr::collect()
  
  nh_table = .convertTranslatedTable(nh_table, translated)
  
  # Define the table reference for the main data
  nh_table_ref = dplyr::tbl(cn(), I(nh_table))
  
  # Fetch all data from the main table
  nh_df = nh_table_ref |> dplyr::collect()
  ...
  if (includelabels) {
    ...
    # add the labels in the metata data
                                   ...
                                   namesonly = TRUE)
{
  ...
  query = metadata_questionnaire_variables |>
    ...
    dplyr::mutate(
      Data.File.Description = substr(Description, 1, nchar),
      Component = paste0(substr(DataGroup, 1, 1), tolower(substr(DataGroup, 2, 20)))
    ) |>
    dplyr::select(Variable.Name,
                  Variable.Description,
                  Data.File.Name = TableName,
                                      ...
                                      details = FALSE)
{
  ...
  query = metadata_questionnaire_descriptions |> 
    ...
    dplyr::mutate(
      Years = paste0(BeginYear, '-', EndYear)
    ) |>
    dplyr::select(
      TableName,
      Years,
                               ...
                               mincategories = 2, details = FALSE, dxa = FALSE)
{
  ...
  # Define the schema-qualified table references
  metadata_variable_codebook = dplyr::tbl(cn(), I(MetadataTable("VariableCodebook")))
  
  # Build the query using dplyr
  query = metadata_variable_codebook |>
    dplyr::filter(TableName == nh_table) |>
    dplyr::select(
      Variable = Variable,
  ...
  if (!is.null(colnames)) {
    query = query |>
      dplyr::filter(Variable %in% colnames)
  }
                             ...
                             namesonly = FALSE)
{
  ...
  query = query |> dplyr::filter(str_detect(tolower(Description.x),tolower(search_terms)))
  
  # Create exclude filter expressions
  if (!is.null(exclude_terms)) {
    exclude_terms = paste(exclude_terms,collapse = "|")
    query = query |> dplyr::filter(!str_detect(tolower(Description.x),tolower(exclude_terms)))
  }
  ...
  if (!is.null(data_group)) {
    if (length(data_group) > 1) {
      query = query |>
        dplyr::filter(str_detect(DataGroup, data_group[1]))
      for (term in data_group[2:length(data_group)]) {
        query = query |>
          dplyr::filter(str_detect(DataGroup, term))
      }
    } else {
      query = query |>
        dplyr::filter(str_detect(DataGroup, data_group))
    }
  }
  ...
  query = query |> dplyr::select(Variable.Name,
                                 Variable.Description,
                                 Data.File.Name,
                                 Data.File.Description,
                                 Begin.Year,
...
.nhanesCodebookDB <- function(nh_table, colname)
{
  ...
  if (!is.null(colname)) {
    query  = query |>
      dplyr::filter(Variable %in% colname)
  }
  
  query = query |> dplyr::select(
    'Variable Name:' = Variable,
    'SAS Label:' = SasLabel,
