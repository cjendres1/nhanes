             ...
             namesonly = FALSE, includerdc = FALSE )
{
  ...
  if(details==FALSE){
    sql = paste0('SELECT "TableName" AS "Data.File.Name",
                "Description" as "Data.File.Description"
                FROM
                "Metadata.QuestionnaireDescriptions" where "DataGroup"=\'',
                               ...
                               nchar=128, namesonly = FALSE)
{
  ...
  sql <- paste0(
    'SELECT DISTINCT V."Variable" AS "Variable.Name",
          SUBSTRING(V."Description" FROM 1 FOR ', nchar, ') AS "Variable.Description",
          V."TableName" AS "Data.File.Name",
          SUBSTRING(Q."Description" FROM 1 FOR ', nchar, ') AS "Data.File.Description",
          "BeginYear" AS "Begin.Year", "EndYear",
          CONCAT(SUBSTRING("DataGroup" FROM 1 FOR 1), LOWER(SUBSTRING("DataGroup" FROM 2 FOR 20))) AS "Component"
  ...
  if(!is.null(data_group)){
    sql = paste0(sql," AND \"DataGroup\" ILIKE '",data_group,"%'")
  }
  df <- .nhanesQuery(sql)
  if(namesonly){
                                      ...
                                      details = FALSE)
{
  sql <- paste0('SELECT DISTINCT "TableName", ',
                '       CONCAT("BeginYear", \'-\', "EndYear") AS "Years", ',
                '"DatePublished" AS "Date.Published" ',
                sprintf('  FROM %s Q', MetadataTable("QuestionnaireDescriptions")),
                sprintf('  WHERE "TableName" LIKE \'%%%s%%\'', pattern))
  if(!is.null(ystart)) {
    sql = paste0(sql, ' AND "BeginYear" >= ', ystart)
  }
  if(!is.null(ystop)) {
    sql <- paste0(sql, ' AND "EndYear" <= ', ystop)
  }
                         ...
                         namesonly = FALSE)
{
  ...
  search_pattern <- if(ignore.case) 'ILIKE' else 'LIKE'
  ...
  sql <- paste0('SELECT V."Variable" AS "Variable.Name", ',
                ...
                sprintf('   FROM %s Q ', MetadataTable("QuestionnaireDescriptions")),
                sprintf('   JOIN %s V ON V."TableName" = Q."TableName" ', MetadataTable("QuestionnaireVariables")),
                sprintf('   WHERE (V."Description" %s \'%%%s%%\'', search_pattern, search_terms[1]))
  
  if (length(search_terms) >= 2) {
    for (term in search_terms[2:length(search_terms)]) {
      sql <- paste0(sql, sprintf(' OR V."Description" %s \'%%%s%%\'', search_pattern, term))
    }
  }
  sql <- paste0(sql, ")")
  ...
  if (!is.null(exclude_terms)) {
    for (term in exclude_terms) {
      sql <- paste0(sql, sprintf(' AND V."Description" NOT %s \'%%%s%%\'', search_pattern, term))
    }
  }
  
  if (!is.null(data_group)) {
    if (length(data_group) > 1) {
      sql <- paste0(sql, ' AND ("DataGroup" ILIKE \'%', data_group[1], "%'")
      for (term in data_group[2:length(data_group)]) {
        sql <- paste0(sql, " OR \"DataGroup\" ILIKE '%", term, "%'")
      }
      sql <- paste0(sql, ")")
    } else {
      sql <- paste0(sql, ' AND "DataGroup" ILIKE \'%', data_group, "%'")
